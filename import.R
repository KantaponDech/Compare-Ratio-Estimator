...
MAEn <- numeric()
RMSEn <- numeric()
estimator <- c("Traditional", "Sisodia and Dwivesdi", "Singh and Kakran", "Upadhyaya and Singh",
               "Singh and Tailor", "Yan and Tian", "Subramani and Kumarpandiyan", "Jerajuddin and Kishun", "Muili and Audu")
...
plot_dataset <- function(){
    ...
    plt = ggplot(map = aes(X,Y, color = "red")) +
      ...
      theme(plot.title = element_text(hjust = 0.5), legend.position = "none", text = element_text(size = 28),
    ...
    for(i in 1:4){
        ...
        plt = ggplot(dat, aes(x = skw, y = val, group = grp, colour = grp)) +
            ...
            theme(plot.title = element_text(hjust = 0.5), text = element_text(size = 28),
                  legend.background = element_rect(fill = "lightblue", size = 1, linetype = "solid", colour = "darkblue"),
                  panel.border = element_rect(colour = "black", fill = NA, size = 1)) +

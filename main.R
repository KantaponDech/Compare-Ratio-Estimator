...
for(skewed in skewed_){ ## Loop by skewness of Y
     ...
     repeat{
          rr <- runif(1, 0.8, 1)
          DataSet <- mvlognormal(n = N, Mu = c(7, 70000), Sigma = c(15, 5 ^ ranY), R = matrix(c(1,rr,rr,1), ncol = 2))
          X <- DataSet[,1]
          Y <- DataSet[,2]
          
          if(!is.na(skewness(Y)) & cor(X,Y) >= 0.8 & cor(X,Y) <= 0.85 & skewness(Y) >= L_skew & skewness(Y) < R_skew){
                setwd("../Project/Plot")
                plot_dataset()
                setwd("../Project")
                break
     ...
     ## Population Detail
     data_detail <- rbind(median(X), median(Y), mean(X), mean(Y), sd(X), sd(Y), kurtosis(X), kurtosis(Y), skewness(X), skewness(Y), cor(X,Y))
     
     row.names(data_detail) <- c("Median X", "Median Y", "Mean X", "Mean Y", "SD X", "SD Y", "Kurtosis X", "Kurtosis Y", "Skewness X", "Skewness Y", "Cor")
     
     write.table(data_detail, sep = ",", append = T, file = "../Project/Detail.csv")
     ...
     for(sampleSize in n_){## Loop by sample size
         ...
         store <- list(store_Trad_Est, store_SisoDwive, store_SinghKa, store_UpaSingh, store_SinghTailor, store_YanTian, store_SubKum, store_JerKis, store_MuiliAudu)
         ...
         write.table(result, sep = ",", append = T, file = "../Project/Result.csv")
